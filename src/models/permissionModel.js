...
exports.assignPermission = (userId, permCode, callback) => {
    ...
    query(checkPermExistSql, [permCode], (existErr, existResult) => {
        ...
        if (existResult.length === 0) {
            const err = new Error(`权限编码 ${permCode} 不存在，请先在permissions表中添加`);
            console.error(err.message);
            return callback(err, null);
        }

        // 第二步：检查该用户是否已拥有此权限
        const checkSql = 'SELECT * FROM user_permissions WHERE user_id = ? AND permission_code = ?';
        query(checkSql, [userId, permCode], (checkErr, checkResult) => {
            ...
            // 未分配则插入关联关系
            const insertSql = 'INSERT INTO user_permissions (user_id, permission_code) VALUES (?, ?)';
...
exports.revokePermission = (userId, permCode, callback) => {
    ...
    const deleteSql = 'DELETE FROM user_permissions WHERE user_id = ? AND permission_code = ?';
...
exports.batchAssignPermissions = (userId, permCodes, callback) => {
    ...
    query(checkSql, permCodes, (checkErr, checkResult) => {
        ...
        // 找出无效权限编码
        const invalidPermCodes = permCodes.filter(code => !validPermCodes.includes(code));
        if (invalidPermCodes.length > 0) {
            const err = new Error(`以下权限编码不存在：${invalidPermCodes.join(', ')}`);
            console.error(err.message);
            return callback(err, null);
        }
        ...
        query(checkUserPermSql, [userId, ...validPermCodes], (userPermErr, userPermResult) => {
            if (userPermErr) {
            ...
            // 提取用户已拥有的权限编码
            const userHadPermCodes = userPermResult.map(item => item.permission_code);
            // 仅插入用户未拥有的权限
            const needAssignCodes = validPermCodes.filter(code => !userHadPermCodes.includes(code));
            ...
            // 第三步：批量插入
            const insertPlaceholders = needAssignCodes.map(() => '(?, ?)').join(',');
            const params = [];
